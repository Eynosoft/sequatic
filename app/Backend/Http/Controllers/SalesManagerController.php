...
class SalesManagerController extends Controller {
    ...
    public function index() {
        ...
        return view('backend::sales_manager.index',['country'=>$country]);
    }
    public function loadSalesManager(Request $request) {
        $salesmanager = $this->user->searchSalesUser($request,self::ROLE)->paginate(10);
        $html = View::make('backend::sales_manager._load-salesmanager', ['salesmanager' => $salesmanager])->render();
        ...
        if ((int) $id) {
            ...
            if (count($model) > 0) {
                $countries = $this->country->findAll();
                 $html = View::make('backend::sales_manager._edit', ['country' => $countries, 'salesmanager' => $model])->render();
                 return Response::Json(['html' => $html]);
            }
            abort(404);
        }
    ...
    public function loadSalesManagerStatics(Request $request) {
