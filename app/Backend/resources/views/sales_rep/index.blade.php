                       ...
                       <div class="row search_section">
                            <div class="col-lg-3">
                                <div class="heading text-center">
                                    <h3>Sales Representative User List</h3>
                                </div>
                            </div>
                            <div class=" col-lg-9 col-md-12">
                                ...
                                <div class="col-lg-5 col-md-5 col-sm-12 ">
                                    <button class="search_btn"  onclick="loadSalesRep(); return false;">Search</button>
                                    <button type="button" class="search_btn color-green" onclick="loadAddModal();"><i class="fa fa-plus"></i>Add Sales Rep</button>
                                </div>
                                </form>
                            </div>
                        </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h3 class="modal-title compose-heading">Add Sales Representative</h3>
                </div>
                <div class="modal-body">
                    <form role="form" class="form-horizontal" name="salesrep_form" action="{{url("backend/sales-rep/create")}}" id="salesrep_form" method="post">
                        <div class="col-lg-12">
                            <div class="form-group">
...
<div aria-hidden="true" aria-labelledby="myModalLabel" data-backdrop="static" data-easein="expandIn" role="dialog" tabindex="-1" id="editSales-rep" class="modal fade modal-style">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button aria-hidden="true" data-dismiss="modal" class="close" type="button">x</button>
                    <h3 class="modal-title compose-heading">Edit Sales Representative</h3>
                </div>
...
<script type="text/javascript">
    ...
    function loadSalesRep(url) {
        //$('#load-general-inquiry').html(loader);
        if (url == 'undefined' || url == '' || url == null) {
            url = '{{url("backend/load-sales-rep")}}';
        }
        $.ajax({
            url: url,
            type: 'post',
    ...
    function loadSalesRepStatics() {
        var url = '{{url("backend/load-sales-rep-statics")}}';
        $.ajax({
            url: url,
            type: 'post',
    ...
    function create() {
        ...
        var url = '{{url("backend/sales-rep/create")}}';
        $.ajax({
            url: url,
            type: 'post',
    ...
    function update() {
        ...
        var url = '{{url("backend/sales-rep/update")}}';
        $.ajax({
            url: url,
            type: 'post',
    ...
    function loadSalesRepEdit(id) {
        ...
        url = '{{url("backend/sales-rep/edit")}}/'+id;
        $.ajax({
            url: url,
            type: 'get',
            //data: $('#searchform').serialize(),
