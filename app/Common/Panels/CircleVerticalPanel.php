...
class CircleVerticalPanel {
    ...
    function __construct($request, $id) {
         ...
         */
        $this->caseOneFirstEquationFormula = ['formula' => '((((0.00444*(0.0361*(w-D))*(D*D*D*D))/400000)/(D/300)) + (((0.22*(0.0361*D)*(D*D*D*D))/400000)/(D/300)))', 'isCubeRoot' => true, 'isRoot' => false];
        $this->caseOneSecondEquationFormula = ['formula' => '(((0.2874*(0.0361*(w-D))*(D*D))/800) + ((0.16*(0.0361*D)*(D*D))/800))', 'isCubeRoot' => false, 'isRoot' => true];

        /**
         * final cot calculation furmula
         */
        $this->final_cost_formula = '((T(3.14*((0.5*D+J)^2)))*M) + ((3.14*((0.5*D+J)^2))*S)';
    }
