...
class LShapePanel3Side {
    ...
    function __construct($request, $id) {
         ...
         */
        $this->final_cost_formula = '(B*L)+((T)*(L+J)*(h+(3*T))*(M))+((L+J)*(h+(3*T))*(S))+(B*F)+((T)*(F+J)*(h+(3*T))*(M))+((F+J)*(h+(3*T))*(S))+((X)*(h+(3*T)))';
    }
    ...
    public function resolveFinalCostFormula($T) {
        ...
        if ($settings) {
            ...
            $search = array("J", "B", "M", "S", "X","T","L","h","F");
            if( $T > $settings['J']){
                $settings['J'] = $T;
            }
            $settings['T'] = (Float)$T;
    ...
    public function get2dValue($viewing_width = null) {
        ...
        if(isset($viewing_width) && !empty($viewing_width)) {
            $t2d = $this->viewing_width / $this->viewing_length;
        } else {
    ...
    public function getValueOfT() {
        ...
        } elseif (($this->viewing_width >= 2 * $this->waterline_height)) {
    ...
    public function resolveCaseFourthFirstEquation() {
        $data2d = $this->get2dValue($this->viewing_width);
        $search = array('h','w','c','f','l');
        $niddle = array($this->viewing_height, $this->waterline_height,$data2d->ref_value_c,$this->viewing_width,$this->viewing_length);
